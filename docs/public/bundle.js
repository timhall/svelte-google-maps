...
assign(Info.prototype, methods$1, protoDev);
...
function encapsulateStyles(node) {
	setAttribute(node, "svelte-41454009", "");
}
...
function create_if_block(state, component) {
	var text_value = state.selected.title, text;
	...
	return {
		...
		p: function update(changed, state) {
			if ((changed.selected) && text_value !== (text_value = state.selected.title)) {
